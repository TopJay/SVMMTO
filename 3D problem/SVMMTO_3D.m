function SVMMTO_3D(nelx,nely,nelz,x0,rmin,E,D,Mf)
close;clc;
...
while beta < betaCn{2}   
   ...
   % MULTI-VARIABLE FILTER
   t = (1:length(D))/(length(D)+1);
   h = (1+tanh((x-t).*beta))/2;
   dh = (1-tanh((x-t).*beta).^2).*beta/2;
   hTilde = (H*h)./Hs;
   hProj = (tanh(beta/2)+tanh(beta*(hTilde-0.5)))./(2*tanh(beta/2));   
   dhProj = beta*(1-tanh(beta*(hTilde-0.5)).^2)./(2*tanh(beta/2));
   % MULTI-MATERIAL INTERPOLATION
   [psiV,dpsiV] = MatIntModel(hProj,dhProj,1);       
   [psiE,dpsiE] = MatIntModel(hProj,dhProj,penal);
   De = psiV*D;
   Ee = Emin+psiE*(E-Emin);
   ...
   dM = dC;
   for j = 1:length(D)
       dC = dC + H*(-(dpsiE{j}*(E-Emin)).*Ce./Hs).*dh(:,j);
       dM = dM + H*(dpsiV{j}*D./Hs).*dh(:,j)/n; 
   end      
   f0val = C;
   df0dx = dC;
   fval = M/Mf-1;
